...
# 量子コンピューティングと量子アルゴリズム {#chapter-intro}

この章では、講義ノートに使用される数学符号や表記法を紹介します。
線形代数（行列のノルム、SVD、特定の行列のプロパティなど）をだいぶ使用され、
ちゃんと理解できますように、読者は自分で付録をざっと読むことを強くお勧めします。

## 量子コンピューティングにおける物理学の知識をさておき

まずサスキンド氏のこの講義[lectures of Susskind](https://www.youtube.com/watch?v=8mi0PoPvLvs)で、
量子力学の「手軽く」の紹介から始めます。
これで、いくつかの考慮事項から始まり、シュレディンガー方程式との関係を直接につながります。
いくつかの数学的計算トリックで、次のセクションで説明する量子力学の4つの公理すべてをざっとの証明方法を説明します。
この紹介を通して、みなさんに小さな物理的な推理から**量子力学の公理**のより深い理解へ導かれることを期待するのでしょう。
これらの「量子力学の公理」という名前は（明らかに）物理学に関連しているように見えるかもしれませんが、
Nielsen and Chuang氏（2002の出版[@NC02]）のかけで、「量子コンピューティングの公理」の定式化はすでに進化したと考えられます。
たぶんスコット・アーロンソン氏が言っていた(https://www.youtube.com/watch?v=SJkbe4Rkv9c)ように、
あなたが物理学者ではないから、量子領域に理解するために量子力学から物理学を取り除く必要があると考えられるのでしょう。

量子コンピューターの学習は量子力学の学習を詳しく掘り下げる必要なく
その4つの公理がたやすく学べることを納得してもらえばよいことにするのはこの章の目的です。

20世紀の初めに、物理学者さんたちが量子現象のモデル化を構築してきたとき、
システム上にそのダイナミックの2つの特性ー
量子システムの時間と空間の進化は（クラシック力学と同じ）連続的で可逆的（クラシック力学と違う）であることが発見されました。
よって、この概念を下記のモデルで形式化して書くことにしました：

量子システムの瞬間状態（時間$p$）は　$\psi(p)$　モデル化とし、
その時間的な $t$ 進化した結果をはある演算子 $U(t)$ でこのシステムを応用した$\psi(p)$とのこと。
この２つの特性の要求は数学的には下記の公式の通り：

-   $U(\epsilon)=I-i\epsilon H$ (連続性条件)
-   $U^\dagger(t)U(t)=I$ (可逆性条件)

一番目の条件は要するに、少しの時間　$\epsilon$　で進化してもこのシステム（$U$ という）自身はほぼ恒等的（アイデンティティ）にし、
変わったはこの小さい部分は別の演算子Hで満足すること。
二番目の条件は、可逆性なので、この $U$ を逆応用するとアイデンティティで、
つまりシステムの状態はかわりしないとのこと。

この２つの条件をあわせば、下記の通りに演算できます：

$$I = U^\dagger(\epsilon)U(\epsilon)= (I+i\epsilon H)(I-i\epsilon H) = I -i \epsilon H + i \epsilon H^\dagger + O(\epsilon^2).$$

よって、この式を満足しなければならない条件は　$H=H^\dagger$　とのことです。

つまり、$H$　という演算子（マトリクス）自身は共役転置という特性があります。
数学上はこのような条件を満足できる演算子はエルミート演算子というです（詳しいは付録にご参考を）！
では、ある量子状態　$\psi(t)$　が　$U(\epsilon)$　との演算子で施されればどうなりますかって問いてみるとー
...
$\epsilion$ はほんのすこしに近づいてくると微分になります。　$$\frac{d}{dt}\psi(t) = -i H \psi(t)$$

これはシュレディンガー数式になります！
定数　($\hbar$)　はさておき、より重要な発見はこの量子システムは微分方程式がみえます。
そうすると、システムの初期状態　$\psi(p)$　を知っておけば完全に決定されます。

正式には、このシステムの解は下記の通りです：

$$\psi(p+t)=e^{-iHt}\psi(p)$$ 

上記の公式のエルミート行列（マトリクス）の指数関数　$e^{-iHt}$　（詳しいは付録にご参考を）で見えますが、
これはエルミートのテイラー展開と定義され、
この関数　$U(t)=e^{-itH}$、実はユニタリ行列てあります。
ユニタリ特性というのはあるベクトルに応用されるとその長さはかわらない、つまり等長性っていうことです。

これで、$\psi(p+t)$ と $\psi(p)$　は長さ固定されたのベクトルとしての量子状態、
実際的に単位ベクトルでに採用されることにし、
表記上、この単位ベクトルはその量子状態に「ケット」と呼び、この方程式を次のように書き直します。

$$\ket{\psi(p+t)}=U(t)\ket{\psi(p)}$$ 

この紹介をきっかけで、次の章節になにか来るのかと、
その量子力学の公理の言い開きをあじわいをしてもらえるのでしょう。
